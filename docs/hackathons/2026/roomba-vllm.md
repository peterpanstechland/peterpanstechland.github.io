...
title: "Roomba-VLLM: A Cyber Butler That Watches My Daughter's Room"
description: "I built a fully local cyber butler on a Jetson Orin NX with an Insta360 Link 2C front camera. He scans my 5-year-old daughter's room every day, generates a cleaning to-do list, and trades stars for weekend allowance — built in 24 hours at the Attrax Spring Hackathon and won the Insta360 Cameraman Award (TOP 1/200 Outlier)."
keywords: [insta360, cameraman, attrax, hackathon, jetson-orin-nx, ollama, gemma, vlm, openclaw, dap, slam, wireguard, homelab, tinkercad, bambu-lab, cyber-butler, link-2c, x5, dit360, ddgs, airsim360]
---

# Roomba-VLLM: A Cyber Butler That Watches My Daughter's Room
...
![The actual robot — Insta360 Link 2 Pro on a Roomba chassis with reComputer J4012 underneath](/img/hackathons/2026/roomba-vllm-robot-hero.png)

---

## The Problem: A 5-Year-Old's Bedroom Looks Like a Typhoon Hit It
...
She never does it after this episode.

What I actually need isn't a smarter nag — it's a **silent supervisor** who walks into my daughter's room, looks around, says "the books on the floor and the stuffed animals on the bed need to go back," waits until she actually does it, double-checks, and gives her a star. Stars become weekend allowance. No yelling. No SaaS. No cloud uploads of my 5-year-old's bedroom.
...
| **Job** | Daily room inspections + to-do list generation + star-based reward system |
| **Personality** | Quiet, methodical, never raises its voice. Logs everything. |
| **Belongs to** | A 5-year-old in our living room |
| **Built in** | 24 hours of real coding (after I burned a day jamming with a robotic arm — see the easter egg at the end) |
...
```mermaid
graph LR
    Cron["⏰ Cron · 4:00 PM<br/>(after kindergarten)"] --> Patrol["🚗 Roomba patrols the room"]
    Patrol --> Capture["📸 Link 2C captures<br/>front camera frames"]
...
Every weekday afternoon, after kindergarten:

1. **Patrol**. The Roomba chassis rolls around the room with the Link 2C gimbaled front camera streaming video back to the Jetson Orin NX.
...
|-----------|-------------|--------------|
| **Insta360 Link 2C** | Front-facing camera with a built-in gimbal — streams video to the Jetson | Auto-tracking gimbal means the butler always frames the room sensibly without me writing tracking code |
| **reComputer J4012 (Jetson Orin NX)** | The local AI brain — runs Ollama + Gemma E4B for Live VLM | Enough VRAM to keep an E4B model warm; small and quiet enough to live in our living room |
| **TinkerCAD-designed enclosure** | Mounts the camera + Jetson + power on a Roomba chassis | Designed in TinkerCAD, printed in PLA on my own Bambu Lab X1C |
| **Roomba chassis** | Mobile base — moves the camera to where my daughter actually leaves things | Already had one; turns "scan a room" into "patrol a room" |
| **Bambu Lab X1C** | 3D printer — printed the enclosure on-site at the hackathon | Brought from home so I could iterate on the mount in real time |

![TinkerCAD enclosure: a flat platform that mounts the Jetson, camera tower, and the cable channel onto the Roomba chassis](/img/hackathons/2026/roomba-vllm-tinkercad-enclosure.png)
...
| **Camera Streaming** | Insta360 Link 2C UVC + custom capture loop | Pulls frames at scan time, hands them to the VLM |
...
graph TB
    subgraph home ["🏠 Home LAN (private)"]
        Roomba["🚗 Roomba chassis<br/>+ TinkerCAD mount"]
        Roomba --> Cam["📷 Insta360 Link 2C<br/>(gimbaled front camera)"]
        Cam -->|"UVC"| Jetson
...
- **Edge (Jetson)** owns perception. It sees the room, runs the VLM, and emits structured findings.
...
The official project window was **48 hours** (April 24 – April 25). I burned the first 24 of them on a robotic arm that had nothing to do with my project (full story in the [easter egg](#easter-egg-the-robotic-arm-that-danced) below). Here's what the *actual* delivery timeline looked like once I started:
...
| **T+3 → T+8** | Wired Link 2C → Jetson over UVC. Got first frames into Gemma E4B locally. |
...
I came to the hackathon planning to demo only the **front-camera + remote teleop** version of the butler. Halfway through Day 2 I learned about Insta360 Research Team's **DAP** (Depth Any Panoramas) — a 360° panoramic depth foundation model — and realized this changes the project's ceiling.

| Capability | Before DAP | With DAP |
|-----------|-----------|----------|
| **Visual coverage** | Front cone (Link 2C) | Full 360° (X5) |
...
- **Eyes** = Insta360 Link 2C (now) → Insta360 X5 (next)
- **Limbs** = the Roomba chassis (and a gimbal on the Link 2C)
...
![Cameraman Award · TOP 1/200 Outlier — special prize on the Attrax Spring Hackathon stage](/img/hackathons/2026/roomba-vllm-cameraman-award.png)
...
5. **Cron + Feishu beats voice UI.** A 5-year-old doesn't need a wake word. She needs a card on Mom's phone that says "your room scan starts in 5 minutes."
6. **Local-only is a feature, not a constraint.** Nothing about my daughter's bedroom should ever leave the LAN. WireGuard is the only door, and only for me.
7. **Hardware events scope themselves.** The moment I learned about DAP, the project's ceiling moved. Don't refuse a unlock just because you're already scoped — re-scope around it.
...
## Easter Egg: The Robotic Arm That Danced

![The hackathon work table — Insta360 X5 box, Antigravity A1 packaging, half-soldered cables, and somewhere off-frame the GaoQing Power robotic arm](/img/hackathons/2026/roomba-vllm-workshop.png)
...
Then came the Live Jam at night — Attrax's all-night DJ set that runs through the hackathon. I rigged the robotic arm to **dance to the music**. Beat detection on the music feed → joint trajectories on the arm. It bobbed, it weaved, it punched the air on the drops. People came over to film it. It was, genuinely, the coolest thing I built that whole week.

It also had nothing to do with my actual hackathon submission.

I went to bed somewhere around 2 AM, woke up at 6, did the math, realized I had **24 hours** until submission, and *then* started building Roomba-VLLM. Everything you read above happened in those 24 hours.
...
> "Books on the floor. Stuffed animals on the bed. You missed the corner."
>
> — The Butler 🤖
